...
-- ─────────────────────────────────────────────────────────────
-- TABELAS DE REFERÊNCIA (sem dependências externas)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- CORREÇÃO 4: Renomeado "status" → "situacoes"
-- POR QUÊ: "status" é uma palavra reservada/sistema no PostgreSQL.
--          Usá-la como nome de tabela não causa erro imediato,
--          mas gera ambiguidades em queries e pode quebrar em
--          versões futuras ou em ORMs (Prisma, TypeORM, etc.).
-- ─────────────────────────────────────────────────────────────
CREATE TABLE situacoes (
    id_situacao   UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    nome_situacao VARCHAR(100) NOT NULL UNIQUE
);
...
CREATE TABLE clientes (
    ...
    -- CORREÇÃO 5a: Coluna adicionada — presente no CSV mas ausente no schema original
    -- POR QUÊ: O CSV (customer_cpf) importaria para cá. Sem a coluna o COPY falharia.
...
CREATE TABLE leads (
    ...
    id_usuario    UUID,
    -- CORREÇÃO 5b: Colunas adicionadas — presentes no CSV mas ausentes no schema original
    assunto_lead                  TEXT,
    data_primeira_interacao_lead  TIMESTAMP,
...
CREATE TABLE negociacoes (
    ...
    -- CORREÇÃO 5c: Coluna adicionada — presente no CSV (negotiation_updated_at)
    data_criacao_negociacao       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    data_atualizacao_negociacao   TIMESTAMP,
    id_estagio                    UUID NOT NULL,
    id_situacao                   UUID NOT NULL,   -- renomeado junto com a tabela
    ...
    CONSTRAINT fk_negociacao_situacao FOREIGN KEY (id_situacao) REFERENCES situacoes (id_situacao)
);

-- Trava de segurança: apenas 1 negociação ATIVA por lead
